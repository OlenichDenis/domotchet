...
//var TplStreet = StreetId
var Lat string
var Lng string
var i = 0
var n = 1000 //количество объектов в запросе с парсера
var owner = 3 // количество совпадений номера телефона для собственника
var timeSleepHendler time.Duration = 60 //минуты
var timeSleepMap time.Duration = 2000  //миллисекунды
var agentPoisk = "РИЕЛТ,АГЕНТ,ПОСРЕД,РИЭЛТ,АГЕНСТ" //слова для поиска в тексте объяв БЕЗ ПРОБЕЛА
...
func handler(city string) (err error) {
	...
	//fmt.Println(sprDistrict, "спр район города")
	fmt.Println(len(sprDistrict), "длина район")
	if len(sprDistrict) == 0{
		fmt.Println("Пустой справочник района")
		return  err
	}
	sprCity := ParsSpr(GetSpr(CityId))
	//fmt.Println(sprCity, "спр город")
	//fmt.Println(len(sprCity), "длина город")
	if len(sprCity) == 0{
		fmt.Println("Пустой справочник города")
		return  err
	}
	//sprStreet := ParsSpr(GetSpr(StreetId))
	//////fmt.Println(sprStreet, "спр улиц")
	//fmt.Println(len(sprStreet), "длина улиц")
	//if len(sprStreet) == 0{
	//	return  err
	//}
	...
	for _, val := range obj.Data {
		...
		if val.Param2313 == "0" || val.Param2836 == "0"{
			continue
		}
		...
		if v, found := sprCity[resultAddress.City]; found {
			...
			resultKey, resultValue, err := createSprValue(resultAddress.City, TplCity, mapSprStack)
			if err != nil{
				continue
			}
		...
		if v, found := sprDistrict[resultAddress.District]; found {
			...
			resultKey, resultValue, err := createSprValue(resultAddress.District, TplDistrict, mapSprStack)
			if err != nil{
				continue
			}
			// добавляем соданный ид-региона в слайс справочника региона
		...
		//	fmt.Println(resultAddress.Street, "улица с парсера", found, "found")
		//} else {
		//	resultKey, resultValue, err := createSprValue(resultAddress.Street, TplStreet, mapSprStack)
		//	fmt.Println(resultKey, resultValue, err, "данные из справочника улиц")
		//	if err != nil{
		//		continue
		//	}
		//	// добавляем соданный ид-региона в слайс справочника региона
		...
		r:= strings.NewReplacer("улица", "ул", "проспект", "пр-кт", "переулок", "пер", "квартал", "кв-л", "шоссе", "ш", "бульвар", "б-р", "проезд", "пр-зд")
		...
		if val.Cat2 == "Комнаты" {
			...
			if v, found := sprRooms[p1945]; found {
				...
				resultKey, resultValue, err := createSprValue(p1945, TplParam1945, mapSprStack)
				if err != nil{
					continue
				}
				// добавляем соданный ид-города в слайс справочника городов
		...
		if val.Cat2 == "Комнаты" {
			...
			if v, found := sprType[val.Param1957]; found {
				...
				resultKey, resultValue, err := createSprValue(val.Param1957, TplParam1957, mapSprStack)
				if err != nil{
					continue
				}
				// добавляем соданный ид-города в слайс справочника городов
		...
		if val.Cat2 == "Квартиры"{
			...
			if v, found := sprHouseType[val.Param2009]; found {
				...
				resultKey, resultValue, err := createSprValue(val.Param2009, TplParam2009, mapSprStack)
				if err != nil{
					continue
				}
				// добавляем соданный ид-города в слайс справочника городов
		...
		if val.Cat2 == "Комнаты"{
			...
			if v, found := sprHouseType[val.Param2567]; found {
				...
				resultKey, resultValue, err := createSprValue(val.Param2567, TplParam2009, mapSprStack)
				if err != nil{
					continue
				}
				// добавляем соданный ид-города в слайс справочника городов
		...
		if mapSend["owner"] == "1" {
			agentPoiskM := strings.Split(agentPoisk, ",")
			descUpp := strings.ToUpper(val.Description)
			fmt.Println(agentPoiskM, "массив слов агент")
			for q:=0; q<len(agentPoiskM); q++{
				fmt.Println(q)
				fmt.Println(agentPoiskM[q], "слово")
				if strings.Contains(descUpp, agentPoiskM[q]){
					fmt.Println(agentPoiskM[q], "совподение")
					mapSend["agent"] = "Нет"
					break
				}else{
					mapSend["agent"] = "Да"
			...
			fmt.Println(mapSend["agent"], " - сотрудничество")
		}
		...
		fmt.Println()
		time.Sleep(timeSleepMap * time.Millisecond)
		...
		fmt.Println("Проверяю идет  после ЖсонР")
		...
		time.Sleep(timeSleepHendler * time.Minute)
		handler(city)
...
func createSprValue(value, tpl string, stack map[string]string) (resKey string, resValue string, err error){
	...
	b, err := ioutil.ReadAll(jsonCity.Body)
	if err != nil{
		return
	}
	...
	err = json.Unmarshal(b, &resultCity)
	if err != nil{
		return
	}

	if len(resultCity.Data) == 0{
		fmt.Sprintf("пустая мапка resultCity", err)
		fmt.Println(err)
		return
	}
	...
	fmt.Println(resKey, resValue, "- отправляем данные в справочник")
